--------------------------------------------------------
--  DDL for Package Body HRBFB3X
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRBFB3X" AS
  ...
  procedure gen_thisheal (json_str_output out clob) AS
    ...
    v_chk               varchar2(1 char);--User37 #6763 24/08/2021
  ...
  begin
    ...
    -->>User37 #6763 24/08/2021
    if not v_found then
      param_msg_error := get_error_msg_php('HR2055', global_v_lang, 'thisheal');
      return;
    end if;
  ...
  procedure gen_thealinf2 (json_str_output out clob) AS
    ...
    --<<User37 #6764 26/08/2021
    v_typeheal          varchar2(10 char) := '!@#$';
    v_dteheal1          number;
    v_dteheal2          number;
    v_codprgheal1       thealcde.codprgheal%type;
    v_codprgheal2       thealcde.codprgheal%type;
    v_flg_codprgheal    varchar2(1 char);
    cursor c_dteheal is
      select b.dteyear, b.dteheal, a.codprgheal, a.typpgm,
             decode(global_v_lang, '101', a.desheale,
                                   '102', a.deshealt,
    ...
    cursor c1 is
      select typeheal,codheal, codprgheal
        from(select '1' typeheal, b.codheal, a.codprgheal
              ...
              where a.dteyear  = b.dteyear
                ...
                and a.codprgheal in (select codprgheal from thealcde where typpgm = '1')
             union
             select '21' typeheal,b.codheal, a.codprgheal
               from thealinf1 a,thealinf2 b
              where a.dteyear  = b.dteyear
                and a.codempid = b.codempid
                and a.codprgheal = b.codprgheal
                and a.codempid = p_codempid
                and a.dteyear  =  nvl(v_dteheal1,1111) and a.codprgheal = v_codprgheal1
             union
             select '22' typeheal,b.codheal, a.codprgheal
               from thealinf1 a,thealinf2 b
              where a.dteyear  = b.dteyear
                and a.codempid = b.codempid
                and a.codprgheal = b.codprgheal
                and a.codempid = p_codempid
                and a.dteyear  =  nvl(v_dteheal2,1111) and a.codprgheal = v_codprgheal2)
      group by typeheal, codheal, codprgheal
      order by typeheal ,codheal, codprgheal;
  ...
  begin
    --<<User37 #6764 26/08/2021
    for i in c_dteheal loop
      v_rcnt        := v_rcnt + 1;
      if v_rcnt = 3 then
        exit;
      end if;
      if v_rcnt = 1 then
        v_dteheal1      := to_number(to_char(i.dteheal,'yyyy'));
        v_codprgheal1   := i.codprgheal;
      else
        v_dteheal2      := to_number(to_char(i.dteheal,'yyyy'));
        v_codprgheal2   := i.codprgheal;
      end if;
    end loop;
    v_rcnt := 0;
    obj_rows      := json_object_t();
    for i in c1 loop
      v_rcnt        := v_rcnt + 1;
      obj_data      := json_object_t();
      if i.typeheal = '1' and v_typeheal = '!@#$' then
        obj_data.put('coderror', '200');
        obj_data.put('codheal', get_label_name('HRBFB3X5',global_v_lang,110));
        obj_rows.put(to_char(v_rcnt - 1), obj_data);
        v_rcnt        := v_rcnt + 1;
        v_typeheal    := i.typeheal;
      elsif (i.typeheal = '21' and v_typeheal <> '21') or (i.typeheal = '22' and v_typeheal <> '22') then
        obj_data.put('coderror', '200');
        obj_data.put('codheal', get_label_name('HRBFB3X5',global_v_lang,120));
        obj_rows.put(to_char(v_rcnt - 1), obj_data);
        v_rcnt        := v_rcnt + 1;
        v_typeheal    := i.typeheal;
      end if;
      ...
      v_rcnt2       := 0;
      if i.typeheal = '1' then
        for v_dteyear in reverse (p_dteyear - 2) .. p_dteyear  loop
          v_rcnt2       := v_rcnt2 + 1;
          begin
            -- 02/12/2022 11:05 redmine #8730
            select 'Y' into v_flg_codprgheal
              from thealinf1 a
             where a.codempid   = p_codempid
               and a.dteyear    = v_dteyear
               and a.codprgheal = i.codprgheal
      ...
      elsif i.typeheal = '21' then
        begin
          select descheck, chkresult
            into v_descheck, v_chkresult
            from thealinf2
           where codempid   = p_codempid
             and codprgheal = v_codprgheal1
             and codheal    = i.codheal
             and dteyear    = v_dteheal1
           fetch first 1 rows only;
        exception when no_data_found then
          v_descheck  := null;
          v_chkresult := null;
        end;
        obj_data.put('descheckO1', v_descheck);
        obj_data.put('chkresultO1', get_tlistval_name('CHKRESUL', v_chkresult, global_v_lang));
      elsif i.typeheal = '22' then
        begin
          select descheck, chkresult
            into v_descheck, v_chkresult
            from thealinf2
           where codempid   = p_codempid
             and codprgheal = v_codprgheal2
             and codheal    = i.codheal
             and dteyear    = v_dteheal2
           fetch first 1 rows only;
        exception when no_data_found then
          v_descheck  := null;
          v_chkresult := null;
        end;
        obj_data.put('descheckO2', v_descheck);
        obj_data.put('chkresultO2', get_tlistval_name('CHKRESUL', v_chkresult, global_v_lang));
      end if;
      obj_rows.put(to_char(v_rcnt - 1), obj_data);
    end loop;
    -------------------------------------------------------------------
        ...
        begin
          ...
          values
              (
                ...
                '',--User37 #6765 24/08/2021  i.desnote, -- item3
