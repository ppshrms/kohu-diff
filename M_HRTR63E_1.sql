--------------------------------------------------------
--  DDL for Package Body M_HRTR63E
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "M_HRTR63E" is
-- last update: 10/02/2021 18:35
 procedure initial_value(json_str in clob) is
    json_obj   json := json(json_str);
  begin
    insert into a(b) values(json_str);commit;
...
procedure gen_tpotentp(json_str_output out clob) is
  ...
  begin
    ...
    obj_row     := json();
--1.select xxx from tvcousr where codcourse
--if xxxx <>
    for r_thistrnn in c_thistrnn loop
      v_rcnt      := v_rcnt+1;
      obj_data    := json();
