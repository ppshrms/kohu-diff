...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "AS_XLSX" 
is
--
...
--
  ...
  is
    ...
    t_sheet pls_integer := nvl( p_sheet, workbook.sheets.count() );
  begin
--<< user22 : 09/01/2018 : STA3600484 || case copy  data to New excel file example  17/03/2017  => 16/03/2013 
    if sysdate > to_date('01/01/2500','dd/mm/yyyy') then 
       --workbook.sheets( t_sheet ).rows( p_row )( p_col ).value := p_value - to_date('01-01-2447','DD-MM-YYYY');
       workbook.sheets( t_sheet ).rows( p_row )( p_col ).value := p_value - to_date('01-01-2447','DD-MM-YYYY') + 1462;
    else   
       --workbook.sheets( t_sheet ).rows( p_row )( p_col ).value :=  p_value - to_date('01-01-1904','DD-MM-YYYY');
       workbook.sheets( t_sheet ).rows( p_row )( p_col ).value := (p_value - to_date('01-01-1904','DD-MM-YYYY')) + 1462;
    end if;  
-->> user22 : 09/01/2018 : STA3600484 || case copy  data to New excel file example  17/03/2017  => 16/03/2013     
    if t_numFmtId is null
...
<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="' || workbook.str_cnt || '" uniqueCount="' || workbook.strings.count() || '">';
    ...
    loop
      t_str := '<si><t>' || dbms_xmlgen.convert( substr( workbook.str_ind( i ), 1, 4000 ) ) || '</t></si>';
      if length( t_tmp ) + length( t_str ) > 4000
      then
        t_xxx := t_xxx || t_tmp;
        t_tmp := null;
      end if;
