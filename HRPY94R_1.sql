--------------------------------------------------------
--  DDL for Package Body HRPY94R
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRPY94R" is
  ...
  begin
    ...
    p_numpf            := to_number(hcm_util.get_string_t(json_obj,'p_numpf'));
    p_typdata          := hcm_util.get_string_t(json_obj,'p_typdata');
    p_mtheffex         := to_number(hcm_util.get_string_t(json_obj,'p_mtheffex'));
    p_yreeffex         := to_number(hcm_util.get_string_t(json_obj,'p_yreeffex'));
    p_typrep           := hcm_util.get_string_t(json_obj,'p_typrep');
    p_typscan          := hcm_util.get_string_t(json_obj,'p_typscan');
  ...
  begin
    ...
    if p_dteprint is null then
      param_msg_error := get_error_msg_php('HR2045', global_v_lang,'dteprint');
      return;
    end if;
    --
...
--
  procedure gen_index(json_str_output out clob)as
    ...
    v_num				number := 0;
    v_numseq			number := 0;
    v_codapp		    varchar2(4000 char) := 'HRPY94R' ;
    ...
    cursor c_emp is
        select a.codempid, a.codcomp, a.numlvl
         ...
         where a.codempid = nvl(p_codempid, a.codempid)
	       ...
	       and	(((p_typdata = 2) and (a.staemp in ('1','3'))) or
	             ((p_typdata = 1) and (a.staemp = '9') and
	              (a.dteeffex between v_stdate and v_endate)))
  ...
  begin
    ...
    del_temp('HRPY94R',global_v_codempid);
    for i in 1..7 loop
      v_typpayt(i) := null;
    end loop;
    ...
    if p_typdata = 1 then
      if p_mtheffex is not null and p_yreeffex is not null then
          v_stdate := to_date('01/'||lpad(p_mtheffex,2,'0')||'/'||lpad(to_char(p_yreeffex),4,'0'),'dd/mm/yyyy');
          v_endate := last_day(v_stdate);
      else
          v_stdate := to_date('01/01/'||lpad(to_char(p_dteyrepay-1),4,'0'),'dd/mm/yyyy');
          v_endate := to_date('31/12/'||lpad(to_char(p_dteyrepay),4,'0'),'dd/mm/yyyy');
      end if;
    end if;
    ...
    begin
      ...
      v_item11 := get_tcompny_name(hcm_util.get_codcomp_level(p_codcomp,1),global_v_lang);
    exception when no_data_found then
      v_item2  := null;
      v_item3  := null;
      v_item12 := null;
    end;
    v_dteprint  := p_dteprint;
	v_item23    := get_nammthful(to_char(v_dteprint,'mm'), global_v_lang);
...
--      v_numseq := 0;
    ...
    for r_emp in c_emp loop
      v_exist := true;
        --v_flgsecu := hcm_secur.get_global_secur(r_emp.codcomp, r_emp.numlvl, global_v_coduser, global_v_numlvlsalst, global_v_numlvlsalen, v_zupdsal);
        v_flgsecu := secur_main.secur1(r_emp.codcomp,r_emp.numlvl,global_v_coduser,global_v_numlvlsalst,global_v_numlvlsalen,v_zupdsal);
      if v_flgsecu then
        v_secur := true;
        v_codempid := r_emp.codempid;

        v_item13 := get_temploy_name(v_codempid,global_v_lang);
        v_item57 := get_tcenter_name(substr(r_emp.codcomp,1,v_tsetcomp),global_v_lang);
        begin
        ...
        if v_codprovc is not null then
           v_item14 := v_item14||' '||get_label_name('HRPY94R',global_v_lang,830)||' '||get_tcodec_name('tcodprov',v_codprovc,global_v_lang);
        end if;
...
--          v_item6 := p_dteyrepay;
          ...
          --Modify 01/09/2552
          begin
            select a.numcotax,a.numcotax,replace(a.adrcomt,chr(10),' '),b.namsign,a.codcompy
            ...
            v_item11 := get_tcompny_name(hcm_util.get_codcomp_level(r1.codcompy,1),global_v_lang);
          exception when no_data_found then
            v_item2  := null;
            v_item3  := null;
            v_item12 := null;
  ...
  -->>user14||STA3600380
          ...
          if (v_temp59 > 0) or (v_temp60) > 0 then
            v_item21 := get_tcodec_name('TCODCERT',v_typpayt(6),global_v_lang);
          end if;
          if (v_temp61 > 0) or (v_temp62 > 0) then
            v_item24 := get_tcodec_name('TCODCERT',v_typpayt(7),global_v_lang);
          end if;
      ...
      begin
        select typsign,codempid,codcomp,
          ...
          into v_typsign,v_codempid2,v_codcomp,
               ...
               v_namsign
          from tsetsign
         where codcompy = hcm_util.get_codcomp_level(nvl(p_codcomp,r_emp.codcomp),1)
           and coddoc = 'HRPY94R';
...
--                       get_tlistval_name('CODTITLE',codtitle,global_v_lang)||namfirstt|| ' ' ||namlastt,
      ...
      insert into ttemprpt
        (codapp, codempid, numseq,
         ...
         temp60,temp61,temp62,item63,item64,item65)
      values
        (v_codapp, global_v_codempid, v_num,
         ...
         v_temp60,v_temp61,v_temp62,v_namsign,v_has_image,p_numpf);
