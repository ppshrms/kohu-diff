--------------------------------------------------------
--  DDL for Package Body HRPY99R
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRPY99R" as
  ...
  procedure get_page_number (p_record_per_page in number, p_sum_page  out number) as
    v_record      number := 0;
    cursor c_grpemp is
      ...
      where t1.codempid = t2.codempid
        ...
        and ((p_typdata = 1) or
	           (p_typdata = 2 and to_number(stddec(t1.amttax,t1.codempid,global_v_chken)) > 0))
  ...
  begin
    ...
    p_dtemthpay  := to_number(hcm_util.get_string_t(obj_detail,'p_dtemthpay'));
    p_typrep     := to_number(hcm_util.get_string_t(obj_detail,'p_typrep'));
    p_typdata    := to_number(hcm_util.get_string_t(obj_detail,'p_typdata'));
...
--      select t1.typinc,(count(distinct t1.codempid)) numrec,t1.codempid,
...
--                     where coduser = global_v_coduser
--                       and t1.codcomp like codcomp||'%')
--        group by t1.codempid,t1.typinc
--        order by t1.codempid,t1.typinc;
      cursor c_tinctxpnd is
        select t1.typinc,(count(distinct t1.codempid)) numrec,t1.codempid,
               nvl(sum(to_number(stddec(t1.amtinc,t1.codempid,global_v_chken))),0) amtinc,
               nvl(sum(to_number(stddec(t1.amttax,t1.codempid,global_v_chken))),0) amttax
...
--               and t1.numlvl between global_v_numlvlsalst and global_v_numlvlsalen 
           and exists(select codcomp
                       ...
                       where coduser = global_v_coduser
                         and t1.codcomp like codcomp||'%')
          group by t1.codempid,t1.typinc
          having ((p_typdata = 1) or
                (p_typdata = 2 and sum(to_number(stddec(t1.amttax,t1.codempid,global_v_chken))) > 0))
          order by t1.codempid,t1.typinc;
...
--      v_numcotax   := split_number_id(r1.numcotax);
    ...
    v_flg_exist := true;
    v_flgsecu := secur_main.secur2(r1.codempid,global_v_coduser,global_v_numlvlsalst,global_v_numlvlsalen,v_zupdsal);
     if v_flgsecu then
    ...
    v_total  := v_amttax_all + nvl(v_addition,0);
    v_resultmod := mod(v_count,8);
    v_resultdivide := floor(v_count/8);

    if v_resultdivide = 0 AND v_resultmod <= 5 then
        v_numpage := 1;
    elsif v_resultmod <= 5 then
    ...
    insert into ttemprpt
        (codempid,codapp   ,numseq  ,
         ...
         item39)
     values
        (global_v_codempid,p_codapp1, 1,
         -- company address --
         p_typrep     ,p_typdata     ,p_codcomp     ,p_typincom  ,
         ...
         v_monthtax   ,v_submission  ,
         -- signature --
         v_name       ,v_desc_codpos ,v_folder      ,v_namsign   ,
         -- tinctxpnd count --
         hcm_util.get_split_decimal(v_amtinc_all,'I'),hcm_util.get_split_decimal(v_amtinc_all,'D'),--to_char(trunc((r1.amtinc - floor(r1.amtinc))*100)),
         hcm_util.get_split_decimal(v_amttax_all,'I'),hcm_util.get_split_decimal(v_amttax_all,'D'),--to_char(trunc((r1.amttax - floor(r1.amttax))*100)),
         hcm_util.get_split_decimal(v_addition,'I'),hcm_util.get_split_decimal(v_total,'I'),-- to_char(v_addition), to_char(v_total)
         -- page count --
         to_char(1 + floor(((v_count + 1)/20))) ,sum_page      ,
--         to_char(mod(v_count,20)+1) ,v_has_image,
         to_char(v_numpage) ,v_has_image,
         hcm_util.get_split_decimal(v_addition,'D'),hcm_util.get_split_decimal(v_total,'D'),
         v_count);-- to_char(v_addition), to_char(v_total)    
    ...
    if v_flg_exist and not v_flg_secure then
      param_msg_error := get_error_msg_php('HR3007',global_v_lang);
      json_str_output := get_response_message('400',param_msg_error,global_v_lang);
      return;
    elsif not v_flg_exist  then
     ...
     if v_flgsecu  then
      ...
      begin
        select decode(global_v_lang,'101',t2.adrrege,
                                      ...
                                      '105',t2.adrreg5,
                                      t2.adrrege) ||' '||
               get_label_name('HRPMC2E1T2',global_v_lang,decode(t2.codprovr,'1000',170,60))||' '||get_tsubdist_name(t2.codsubdistr,global_v_lang) address,
               ...
               get_tcodec_name('TCODCNTY',t2.codcntyr,global_v_lang)||' '||
               t2.codpostr address2,
               get_tlistval_name('CODTITLE',t1.codtitle,global_v_lang)||t1.namfirstt,
               t1.namlastt,
--               get_label_name('HRPY99R',global_v_lang,150)||' '||get_tlistval_name('CODTITLE',t1.codtitle,global_v_lang)||t1.namfirstt,
--               get_label_name('HRPY99R',global_v_lang,160)||' '||t1.namlastt,
               t2.numoffid,
               t3.numtaxid,
               t3.flgtax
          into v_address,
      ...
      --
      begin
        select typpayroll into v_typpayroll
          from tinctxpnd
         where codcomp like p_codcomp||'%'
           and codempid  = r_tinctxpnd.codempid
           and dteyrepay = (p_dteyrepay - global_v_zyear)
           and dtemthpay = p_dtemthpay
           and codpay    = r_tinctxpnd.codpay
      ...
      obj_data.put('dtepay',to_char(v_date01,'dd/mm/yyyy'));
        ...
        insert into ttemprpt
         ...
         values
            (global_v_codempid,p_codapp2,v_numseq,
             p_dteyrepay,p_dtemthpay,p_typrep,p_typdata,p_codcomp,p_typincom ,
             v_numoffid,v_numtaxid,v_flgtax,v_namfirstt,v_namlastt ,
             to_char(1 + floor(((v_count + 1)/20))) ,v_numrec ,
             to_char(mod(v_count,20)+1),
             v_item05,
             hcm_util.get_split_decimal(v_temp02,'I'),
             v_temp03,
             hcm_util.get_split_decimal(v_temp04,'I'),
             v_address,
             hcm_util.get_split_decimal(v_temp02,'D'),hcm_util.get_split_decimal(v_temp04,'D'),
             -- signature --
             v_name,v_desc_codpos,v_folder,v_namsign,null,
             sum_page,null,
             to_char(v_date01, 'DD/MM/') || (to_number(to_char(v_date01, 'YYYY')) + hcm_appsettings.get_additional_year),
             v_numcotax,v_has_image,v_address2,'TABLE');

--        v_year      := hcm_appsettings.get_additional_year;
--        v_dtecalc   := to_date(hcm_util.get_string(obj_data, 'dtecalc'), 'DD/MM/YYYY');
      ...
      begin
        insert into ttemprpt
         ...
         values
            (global_v_codempid,p_codapp2,v_numseq,
             p_dteyrepay,p_dtemthpay,p_typrep,p_typdata,p_codcomp,p_typincom ,
             null,null,v_flgtax,null,null ,
             to_char(1 + floor(((v_count + 1)/20))) ,v_numrec ,
             to_char(mod(v_count,20)+1),
             null,
             hcm_util.get_split_decimal(v_amtinc_all,'I'),
             v_temp03,
             hcm_util.get_split_decimal(v_amttax_all,'I'),
             v_address,
             hcm_util.get_split_decimal(v_amtinc_all,'D'),hcm_util.get_split_decimal(v_amtinc_all,'D'),
             -- signature --
             v_name,v_desc_codpos,v_folder,v_namsign,null,
             sum_page,null,
             to_char(v_date01, 'DD/MM/') || (to_number(to_char(v_date01, 'YYYY')) + hcm_appsettings.get_additional_year),
