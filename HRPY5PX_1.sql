--------------------------------------------------------
--  DDL for Package Body HRPY5PX
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRPY5PX" is
...
--    end if;
  ...
  procedure gen_detail (json_str_output out clob) is
    ...
    v_codapp      varchar2(30 char) := 'HRPY5PX';
    v_amtincom    number  := 0;
  ...
  begin
    ...
    v_stmt := v_stmt || to_clob('
      ...
      where dteyrepay = ''' || to_char(p_dteyrepay) || '''
        ...
        and codempid in (
          select codempid
            from ttaxcur
           where codempid  = tsincexp.codempid
             and dteyrepay = tsincexp.dteyrepay
             and dtemthpay = tsincexp.dtemthpay
             and numperiod = tsincexp.numperiod
             and nvl(stddec(amtnet, codempid, ''' || v_chken || '''), 0) < 0
        )
    ...
    while dbms_sql.fetch_rows(curid) > 0 loop
      ...
      if v_flgsecu then
        ...
        begin
          select avg(stddec(amtincom1,codempid,v_chken))
            into v_amtincom
            from ttaxcur
    ...
    --<< add  By: User25/Thanittha.y Date: 12/05/2018   ErrorNo: STA4610117
      begin
