--------------------------------------------------------
--  DDL for Package Body HRESH6X
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRESH6X" AS
  ...
  procedure gen_thistrnn (json_str_output out clob) AS
    obj_data            json_object_t;
    v_flgeval           ttrimph.flgeval%type := 'U';
    v_desc_flgtrain     varchar2(100 char) := get_label_name('HRESH6X3', global_v_lang, 430);
  ...
  begin
    ...
    for i in c1 loop
      ...
      if i.flgtrain = 'Y' then
        v_desc_flgtrain := get_label_name('HRESH6X3', global_v_lang, 430);
      else
        v_desc_flgtrain := get_label_name('HRESH6X3', global_v_lang, 440);
      end if;
      obj_data.put('codempid', i.codempid);
      obj_data.put('codcomp', i.codcomp);
      obj_data.put('desc_codcomp', get_tcenter_name(i.codcomp, global_v_lang));
  ...
  procedure gen_thistrnb (json_str_output out clob) AS
    ...
    cursor c1 is
        --<<User37 #4824 8. ES.MS Module (P2) 08/03/2021 
        select descomment
          from thisclsss
         where dteyear  = p_dteyear
