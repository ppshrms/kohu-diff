--------------------------------------------------------
--  DDL for Package Body HRAL1OX
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRAL1OX" as
  ...
  begin
    ...
    if param_msg_error is null then
      obj_row  := json_object_t();
      v_stdate := to_date('01/01/'||to_char(p_year),'dd/mm/yyyy');
      ...
      for r1 in c_tgrpplan loop
        ...
        if r1.codcomp <> o_codcomp then
         ...
         obj_data := json_object_t();
         obj_data.put('coderror','200');
         obj_data.put('codcomp', nvl(v_codcomp, ''));
         obj_data.put('desc_codcomp', get_tcenter_name(v_codcomp, global_v_lang));
         obj_data.put('codcalen', nvl(v_codcalen, ''));
         obj_data.put('desc_codcalen', nvl(get_tcodec_name('TCODWORK',v_codcalen,global_v_lang), ''));
    ...
    if param_msg_error is not null then
      ...
      if isInsertReport then
        ...
        if param_msg_error is null then
          insert_report_comp(obj_row,'groupplan');
        end if;
      else
        json_str_output := obj_row.to_clob;
      end if;
  ...
  procedure get_groupmonth(json_str_input in clob, json_str_output out clob, flgchk varchar2 default 'Y') is
    obj_row         json_object_t;
    obj_data        json_object_t;
    v_stdate		    date;
    v_endate		    date;
    ...
    v_month2        varchar2(10 char);
    o_month         varchar2(10 char) := '!@#$';
    o_month2        varchar2(10 char) := '!@#$';
    cursor c_tgrpplan is
      select to_char(dtework,'MM/YYYY') month_year
            from tgrpplan p
          where codcomp = p_codcomp
            and codcalen = nvl(p_codcalen,codcalen)
            and to_char(dtework,'YYYY') = p_year
        group by to_char(dtework,'MM/YYYY')
        order by to_char(dtework,'MM/YYYY');
    ...
    select codcomp,dtework,codcalen,typwork,codshift
       ...
       where codcomp 	= p_codcomp
         and codcalen = nvl(p_codcalen,codcalen)
         and dtework  between v_stdate2 and v_endate2
    order by dtework;
  ...
  begin
    ...
    if param_msg_error is null then
      ...
      for r1 in c_tgrpplan loop
        v_row := v_row + 1;
        v_stdate2   := to_date('01/'||r1.month_year,'dd/mm/yyyy');
        v_endate2   := last_day(v_stdate2);
        v_month2    := to_char(v_stdate2, 'MM');
        v_month     := get_nammthful(v_month2, global_v_lang);

        for r2 in c_tgrpplan2 loop
          v_day := v_day + 1;
          if v_month2 <> o_month then
          ...
          obj_data.put('month', lpad(to_char(v_month2),2,'0'));
          obj_data.put('desc_month', v_month);
          obj_data.put('year', p_year);
...
--        end if;
        if param_msg_error is null then
         insert_report_comp(obj_row,'groupmonth');
        end if;
      else
        json_str_output := obj_row.to_clob;
      end if;
  ...
  begin
    ...
    for i in c1 loop
     v_row := v_row + 1;
      ...
      obj_data.put('timstrtw', substr(i.timstrtw, 1, 2)||':'||substr(i.timstrtw, 3, 2));
      obj_data.put('timendw', substr(i.timendw, 1, 2)||':'||substr(i.timendw, 3, 2));
      obj_data.put('timshift', substr(i.timstrtw, 1, 2)||':'||substr(i.timstrtw, 3, 2)||' - '||substr(i.timendw, 1, 2)||':'||substr(i.timendw, 3, 2));
      obj_data.put('qtydaywk', i.qtydaywk);
      obj_row.put(to_char(v_row-1), obj_data);
    end loop;
  ...
  procedure insert_report_comp(obj_row in json_object_t,insert_type in varchar2) is
    obj_data            json_object_t;
    obj_temp            json_object_t;
    v_numseq            number := 0;
    v_emp_count         number := 0;
    json_str_output     clob;
    t_codapp            varchar2(10 char);

--    v_codcomp                 varchar2(1000 char);
--    v_codcalen                varchar2(1000 char);
  begin

  if insert_type = 'groupplan' then
      t_codapp := p_codapp || '1';
  else
      t_codapp := p_codapp || '2';
  end if;
    ...
    if obj_row.get_size > 0 then
      ...
      for i in 0 .. obj_row.get_size - 1 loop
        ...
        if insert_type = 'groupplan' then
          obj_data.put('r_codapp', t_codapp);
        ...
        if insert_type = 'groupplan' then
          get_groupmonth('', json_str_output,'N');
        end if;
  ...
  procedure insert_ttemprpt_comp(v_numseq in number, obj_data in json_object_t) is
    ...
    v_item35            ttemprpt.item35%type;   v_item36            ttemprpt.item35%type;
    ...
    r_codapp            varchar2(10 char);      v_year              varchar2(10 char);

  begin
    v_item33 := hcm_util.get_string_t(obj_data, 'codempid');
    v_item35 := hcm_util.get_string_t(obj_data, 'desc_codempid');
    v_item32 := hcm_util.get_string_t(obj_data, 'codcomp');
    v_item34 := hcm_util.get_string_t(obj_data, 'codcalen');
    v_item36 := hcm_util.get_string_t(obj_data, 'desc_month');
    ...
    r_codapp := hcm_util.get_string_t(obj_data, 'r_codapp');
--    v_year   := to_char(to_number(p_year) + hcm_appsettings.get_additional_year);
      ...
      insert
        into ttemprpt
          (
            ...
            item71, item37, item73
          )
      values
          (
            global_v_codempid, r_codapp, b_numseq,
  ...
  begin
    ...
    --day name--
    v_item38  := get_label_name('HRAL1OXC1', global_v_lang, '10');
    v_item39  := get_label_name('HRAL1OXC1', global_v_lang, '20');
    v_item40  := get_label_name('HRAL1OXC1', global_v_lang, '30');
    v_item41  := get_label_name('HRAL1OXC1', global_v_lang, '40');
    v_item42  := get_label_name('HRAL1OXC1', global_v_lang, '50');
    v_item43  := get_label_name('HRAL1OXC1', global_v_lang, '60');
    v_item44  := get_label_name('HRAL1OXC1', global_v_lang, '70');
