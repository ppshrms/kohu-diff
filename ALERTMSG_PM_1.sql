--------------------------------------------------------
--  DDL for Package Body ALERTMSG_PM
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "ALERTMSG_PM" is
...
begin
  ...
  gen_probationn(null,'Y',null,null);
  gen_probationp(null,'Y',null,null);
  ...
  for i in c_tpmalert loop
    ...
    if v_testsend = 'Y' then
      ...
      loop
        ...
        if v_count = 0 then
           p_lang          := chk_flowmail.get_emp_mail_lang(v_codempid);
      ...
      if v_typesend = 'A' then
        if v_seq > 0 then
            for r_group in c_group_sendmail loop
                ...
                p_lang          := chk_flowmail.get_emp_mail_lang(v_receiver);

                v_data_table    := null;
                v_data_list     := null;
                ALERTMSG_PM.gen_table_header(p_codcompy,v_mailalno,v_dteeffec);

                for i in c_sendmail loop
                    v_numseq := v_numseq + 1;
                    ALERTMSG_PM.gen_table(p_codcompy,v_mailalno,v_dteeffec,' codempid = '''||i.codempid||''' ',null,v_numseq,20);
--                    ALERTMSG_PM.gen_file(p_codcompy,v_mailalno,v_dteeffec,' codempid = '''||i.codempid||''' ',v_numseq,20);
                end loop;
...
--      end if;
      ...
      if v_typesend = 'A' then
        for r_group in c_group_sendmail loop
            ...
            for i in c_sendmail loop
                v_numseq := v_numseq + 1;
                ALERTMSG_PM.gen_table(p_codcompy,v_mailalno,v_dteeffec,' codempid = '''||i.codempid||''' ',null,v_numseq,22);
--                ALERTMSG_PM.gen_file(p_codcompy,v_mailalno,v_dteeffec,' codempid = '''||i.codempid||''' ',v_numseq,22);
            end loop;
...
--      end if;
      ...
      if v_typesend = 'A' then
        for r_group in c_group_sendmail loop
            ...
            for i in c_sendmail loop
                v_numseq := v_numseq + 1;
                ALERTMSG_PM.gen_table(p_codcompy,v_mailalno,v_dteeffec,' ttprobat.codempid = '''||i.codempid||''' and ttprobat.dteduepr = to_date('''||i.dteduepr||''',''dd/mm/yyyy'') ',null,v_numseq,24);
--                ALERTMSG_PM.gen_file(p_codcompy,v_mailalno,v_dteeffec,' ttprobat.codempid = '''||i.codempid||''' and ttprobat.dteduepr = to_date('''||i.dteduepr||''',''dd/mm/yyyy'') ',v_numseq,24);
            end loop;
...
--
procedure gen_table_header (p_codcompy in varchar2,
                    p_mailalno in varchar2,
                    p_dteeffec in date) is
...
begin
    ...
    for i in c_tpmparam loop
      ...
      p_column_label(v_index - 1)   := i.descript;
    end loop;
    -- cal width column
    ...
    v_max_col_global    := v_max_col;
    v_data_table := v_data_table||'<tr class="TextBody" bgcolor="#006699">
                         <td width="'||p_column_width(1)||'%"  height="20" align="center"><font color="#FFFFFF">'||get_label_name('ESS', p_lang, 20)||'</font></td>';
...
end; -- gen_table_header

procedure gen_table (p_codcompy in varchar2,
                    p_mailalno in varchar2,
                    p_dteeffec in date,
                    p_where    in varchar2,
...
--  v_funcdesc     varchar2(2000 char);
  ...
  cursor c1 is
    select b.fparam,b.ffield,
           b.codtable,decode(b.codtable,'V_TEMPLOY',nvl(p_where2,p_where),p_where) fwhere,
           'select '||b.ffield||' from '||b.codtable||' where '||decode(b.codtable,'V_TEMPLOY',nvl(p_where2,p_where),p_where) as stm ,flgdesc
      from tpmparam b
     where codcompy = p_codcompy
       and mailalno = p_mailalno
       and dteeffec = p_dteeffec
...
begin
  ...
  if v_statment is not null then
    ...
    if p_typemail in ('20','22','70','80','90','100','120','130') then
      v_stment := 'select '||v_statment||' from v_temploy where '||p_where;
    elsif p_typemail in ('110') then
      v_stment := 'select '||v_statment||' from v_temploy, tappldoc where tappldoc.numappl = v_temploy.numappl and '||p_where;
...
begin
  ...
  for i in c_tpmalert loop
    ...
    if v_testsend = 'Y' then
      ...
      if i.qtydayb is not null then -- Before
         v_endate := to_char((sysdate + i.qtydayb),'dd/mm/yyyy');
         v_where  := v_where||' and  ttmovemt.dteeffec = to_date('''||v_endate||''',''dd/mm/yyyy'') ';
      end if;
      ...
      v_stment := 'select ttmovemt.codempid,to_char(ttmovemt.dteeffec,''dd/mm/yyyy'') dteeffec,ttmovemt.numseq '||
                  'from ttmovemt ,temploy1 ' ||
                  ' where ttmovemt.staupd   in (''U'',''C'') '||
                  ' and ttmovemt.codempid = temploy1.codempid '||
                  ' and temploy1.staemp in (''1'',''3'') '||
                  ' and temploy1.codcomp like '''|| p_codcompy ||'%'' '||
                  v_where|| ' and rownum <3 '||
      ...
      loop
        ...
        if v_count = 0 then
          ...
          for k in c_tobfcde loop
            ...
            check_ttmovemt( v_codempid,to_date(data_file(2),'dd/mm/yyyy'),data_file(3),
...
--              arr_qtywidrwo(v_arr)     := v_qtywidrwo;
          ...
          if v_arr > 0 then
            ...
            if v_typesend = 'A' then
              if v_numseq > 0 then
                ALERTMSG_PM.gen_file(p_codcompy,i.mailalno,i.dteeffec,' ttmovemt.codempid = '''||v_codempid||''' and ttmovemt.dteeffec = '''||data_file(2)||''' and ttmovemt.numseq = '''||data_file(3)||''' ',v_seq,i.typemail);
              end if;
            else
              v_mail_message := null;
...
--
                v_mail_message := v_mail_message||'<tr class=TextBody>
                <td width=100 align=center>'||arr_codobf(n)||'</td></tr>';
              end loop;
...
--                                  </tr>'||v_mail_message;
            ...
            if flg_log = 'Y' then
              insert_talertlog(p_codcompy,i.mailalno,i.dteeffec,v_codempid,data_file(3),null,null,null,to_date(data_file(2),'dd/mm/yyyy'),null,null);
            end if;
          end if;
        end if;--v_count
      end loop;
      ...
      if v_typesend = 'A' then
        if v_seq > 0 then
          ...
          if v_numseq > 0 then
            excel_mail(v_item,v_label,null,'AUTO',p_codapp_file,v_filename); --user36 STA3590309 05/09/2016 excel_mail(v_item,v_label,null,'AUTO',v_codapp,'auto');
          end if;
...
--      end if;
      ...
      if v_typesend = 'A' then
        for r_group in c_group_sendmail loop
            ...
            for i in c_sendmail loop
                v_numseq := v_numseq + 1;
                ALERTMSG_PM.gen_table(p_codcompy,v_mailalno,v_dteeffec,' ttmovemt.codempid = '''||i.codempid||''' and ttmovemt.dteeffec = to_date('''||i.dteeffec||''',''dd/mm/yyyy'') and ttmovemt.numseq = '''||i.numseq||''' ',' codempid = '''||i.codempid||''' ',v_numseq,40);
--                ALERTMSG_PM.gen_file(p_codcompy,v_mailalno,v_dteeffec,' ttmovemt.codempid = '''||i.codempid||''' and ttmovemt.dteeffec = to_date('''||i.dteeffec||''',''dd/mm/yyyy'') and ttmovemt.numseq = '''||i.numseq||''' ',v_numseq,40);
            end loop;
...
--      end if;
      ...
      if v_typesend = 'A' then
        for r_group in c_group_sendmail loop
            ...
            for i in c_sendmail loop
                v_numseq := v_numseq + 1;
                ALERTMSG_PM.gen_table(p_codcompy,v_mailalno,v_dteeffec,' ttmovemt.codempid = '''||i.codempid||''' and ttmovemt.dteeffec = to_date('''||i.dteeffec||''',''dd/mm/yyyy'') and ttmovemt.numseq = '''||i.numseq||''' ',' codempid = '''||i.codempid||''' ',v_numseq,42);
--                ALERTMSG_PM.gen_file(p_codcompy,v_mailalno,v_dteeffec,' ttmovemt.codempid = '''||i.codempid||''' and ttmovemt.dteeffec = to_date('''||i.dteeffec||''',''dd/mm/yyyy'') and ttmovemt.numseq = '''||i.numseq||''' ',v_numseq,42);
            end loop;
...
--                                v_typesend);
      ...
      if v_typesend = 'A' then
        for r_group in c_group_sendmail loop
            ...
            for i in c_sendmail loop
                v_numseq := v_numseq + 1;
                ALERTMSG_PM.gen_table(p_codcompy,v_mailalno,v_dteeffec,' ttprobat.codempid = '''||i.codempid||''' and ttprobat.dteduepr = to_date('''||i.dteduepr||''',''dd/mm/yyyy'') ',null,v_numseq,44);
--                ALERTMSG_PM.gen_file(p_codcompy,v_mailalno,v_dteeffec,' ttprobat.codempid = '''||i.codempid||''' and ttprobat.dteduepr = to_date('''||i.dteduepr||''',''dd/mm/yyyy'') ',v_numseq,44);
            end loop;
...
procedure gen_public_holiday (p_codcompy  in varchar2,
  ...
  v_namimage    tpmpublic.namimage%type;
  v_namimage_path   varchar2(1000);
  ...
  cursor c_tpmpublic is
    select dtepublic,subject,message,filename,namimage
      from tpmpublic
     where codcompy = p_codcompy
...
begin
  ...
  for i in c_tpmalert loop
    ...
    if v_testsend = 'Y' then
      ...
      v_mailalno  := i.mailalno;
      v_dteeffec  := i.dteeffec;
      v_dtepublic := to_date(v_endate,'dd/mm/yyyy');
      v_data := 'N';
      ...
      for j in c_tpmpublic loop
        ...
        v_namimage  := j.namimage;
        if v_namimage is not null then
            v_namimage_path := get_tsetup_value('BACKEND_URL')||get_tsetup_value('PATHDOC')||get_tfolderd('HRPMATE2')||'/'||v_namimage;
        end if;
        exit;
      end loop;
      ...
      if v_data = 'Y' then
        ...
        loop
          ...
          if v_count = 0 then
            ...
            p_lang          := chk_flowmail.get_emp_mail_lang(v_codempid);
            sendmail_public( v_codempid,i.mailalno,i.dteeffec,v_dtepublic,i.codsend,i.typemail,v_subject,v_message||i.message,v_namimage_path,v_typesend);
            if flg_log = 'Y' then
...
--  end if;
  ...
  if v_email is not null then
    ...
    v_msg      := replace(v_msg ,'[PARAM1]', get_temploy_name(p_codempid,p_lang));
    --<<user36 STA3590309 09/09/2016
--     v_msg := 'From: ' ||v_sender_email||  crlf ||
--                  'To: '||v_email||crlf||
--                  'Subject: '||p_subject||crlf||
--                  'Content-Type: text/html;'||crlf||crlf||v_msg;
--    v_error    := send_mail(v_email,v_msg);
    v_error    := SendMail_AttachFile(v_sender_email,v_email,p_subject,v_msg,p_filename,null,null,null,null);
    ...
    if v_testsend = 'Y' then
      ...
      if v_typesend = 'A' then
        for r_group in c_group_sendmail loop
            ...
            for i in c_sendmail loop
                v_numseq := v_numseq + 1;
                ALERTMSG_PM.gen_file(p_codcompy,v_mailalno,v_dteeffec,' ttexempt.codempid = '''||i.codempid||''' and ttexempt.dteeffec = to_date('''||i.dteeffec||''',''dd/mm/yyyy'') ',v_numseq,60);
            end loop;
...
--         v_where  := v_where||' and  dteretire =  to_date('''||v_endate||''',''dd/mm/yyyy'') ';
      ...
      if v_typesend = 'A' then
        for r_group in c_group_sendmail loop
            ...
            for i in c_sendmail loop
                v_numseq := v_numseq + 1;
                ALERTMSG_PM.gen_file(p_codcompy,v_mailalno,v_dteeffec,' codempid = '''||i.codempid||'''  ',v_numseq,70);
            end loop;
    ...
    if v_testsend = 'Y' then
      ...
      if v_typesend = 'A' then
        for r_group in c_group_sendmail loop
            ...
            for i in c_sendmail loop
                v_numseq := v_numseq + 1;
                ALERTMSG_PM.gen_file(p_codcompy,v_mailalno,v_dteeffec,' codempid = '''||i.codempid||'''  ',v_numseq,80);
            end loop;
    ...
    if v_testsend = 'Y' then
      ...
      if v_typesend = 'A' then
        for r_group in c_group_sendmail loop
            ...
            for i in c_sendmail loop
                v_numseq := v_numseq + 1;
                ALERTMSG_PM.gen_file(p_codcompy,v_mailalno,v_dteeffec,' codempid = '''||i.codempid||'''  ',v_numseq,90);
            end loop;
    ...
    if v_testsend = 'Y' then
      ...
      if v_typesend = 'A' then
        for r_group in c_group_sendmail loop
            ...
            for i in c_sendmail loop
                v_numseq := v_numseq + 1;
                ALERTMSG_PM.gen_file(p_codcompy,v_mailalno,v_dteeffec,' codempid = '''||i.codempid||'''  ',v_numseq,100);
            end loop;
...
begin
  ...
  for i in c_tpmalert loop
    ...
    if v_testsend = 'Y' then
      ...
      V_STMENT := 'select distinct temploy1.codempid from temploy1, tappldoc '
                    ||V_WHERE|| ' order by temploy1.codempid';
      ...
      if v_typesend = 'A' then
        for r_group in c_group_sendmail loop
            ...
            for i in c_sendmail loop
                v_numseq := v_numseq + 1;
                ALERTMSG_PM.gen_file(p_codcompy,v_mailalno,v_dteeffec,' v_temploy.codempid = '''||i.codempid||'''  ',v_numseq,110);
            end loop;
    ...
    if v_testsend = 'Y' then
      ...
      if v_typesend = 'A' then
        for r_group in c_group_sendmail loop
            ...
            for i in c_sendmail loop
                v_numseq := v_numseq + 1;
                ALERTMSG_PM.gen_file(p_codcompy,v_mailalno,v_dteeffec,' codempid = '''||i.codempid||''' ',v_numseq,120);
            end loop;
    ...
    if v_testsend = 'Y' then
      ...
      if v_typesend = 'A' then
        for r_group in c_group_sendmail loop
            ...
            for i in c_sendmail loop
                v_numseq := v_numseq + 1;
                ALERTMSG_PM.gen_file(p_codcompy,v_mailalno,v_dteeffec,' codempid = '''||i.codempid||''' ',v_numseq,130);
            end loop;
...
--  end if;
 -- end if;
  ...
  if p_typemail = '10' then
    begin
        select get_tsetup_value('PATHAPI')||get_tsetup_value('PATHDOC')||get_tfolderd('HRPMATE')||'/'||NAMIMAGE
          into v_filename
          from ( select NAMIMAGE
                   from TPMIMAGEBD
...
--  end if;
  ...
  if v_data_table is not null then
    v_data_table := '<table width="100%" border="0" cellpadding="0" cellspacing="1" bordercolor="#FFFFFF">'||v_data_table||'</table>';
