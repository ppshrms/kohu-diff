--------------------------------------------------------
--  DDL for Package Body HRAL2JE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRAL2JE" is
  -- update 08/02/2023
  ...
  begin
    ...
    p_codcomp_query := hcm_util.get_string_t(json_obj,'p_codcomp_query');
...
--         where codcomp like p_codcomp||'%'
		...
		and		exists(	select  a.codempid
                         ...
                         where b.codcomp 	like nvl(p_codcomp,s_codcomp)||'%' -- where b.codcomp 	like p_codcomp||'%'
--<<error IPO implement 24/02/2023
                            and exists (select codcomp  from tusrcom xx
                                              where  xx.coduser = global_v_coduser
                                                  and  a.codcomp like xx.codcomp||'%')
-->>error IPO implement 24/02/2023
                           ...
                           and  p.codcomp = (select  max(e.codcomp)
                                               from   tgrpwork e
                                               where  b.codcomp like e.codcomp||'%'  --<<user46 #6727 01/10/2021
...
--error IPO implement 24/02/2023        
            ...
            --obj_data.put('codshift'||lpad(to_char(v_day),2,'0'), nvl(r2.codshift, ''));
                    ...
                    obj_data.put('dayedit',global_v_day_edit);
                    obj_row.put(to_char(v_row-1),obj_data);
      end loop;
  ...
  procedure get_groupemp(json_str_input in clob, json_str_output out clob) is
    ...
    s_codcomp     varchar2(50 char);
    v_nameemp     varchar2(400 char) := '!@#$';
	cursor c_temploy1 is
    select codempid,codcomp,numlvl,dteempmt,dteeffex
      from temploy1 a
     where 1 = 1 --<<user46 #6727 01/10/2021 codcomp like p_codcomp_query||'%'
       --and get_tgrpwork_codcomp(get_tattence_codcomp(codempid,v_stdate,v_endate),null) = p_codcomp
...
--                    and b.codcomp like p_codcomp_query||'%' --<<user46 #6727 01/10/2021
                    and b.codcomp like nvl(p_codcomp,s_codcomp)||'%' --<<user4 20/04/2023
                    and b.codcomp like p_codcomp_query||'%' --<<user4 20/04/2023
                    and p_codcomp = (select  max(e.codcomp)
                                       from   tgrpwork e
    ...
    if param_msg_error is null then
      ...
      for r1 in c_temploy1 loop
        ...
        if v_flgpass then
          ...
          v_nameemp   := get_temploy_name(v_codempid,global_v_lang) ;
          for r2 in c_tattence loop
            ...
            if r1.codempid <> o_codempid then
              ...
              obj_data.put('desc_codempid',v_nameemp);
              ...
              loop
                ...
                obj_data.put('codcomp'||to_char(v_date,'dd'),'');
            ...
            obj_data.put('month', lpad(to_char(p_month),2,'0'));
            obj_data.put('year', p_year);
            obj_data.put('codcomp'||to_char(r2.dtework,'dd'),nvl(r2.codcomp, ''));
  ...
  cursor c1 is
    select a.codempid,a.codcomp,a.dtework,a.codcalen,a.typwork,a.codshift,b.numlvl,to_char(dtework,'dy') daynam
       ...
       --> Somchai || SEA-HR2201 || 23022023
       and a.codempid = nvl(p_codempid, a.codempid)
       --> Somchai || SEA-HR2201 || 23022023
       and a.dtework between v_stdate and v_endate;

  cursor c_old_tattence is
    select codempid, dtework, codcalen, codcomp, dtestrtw, dteendw, typwork, codshift, hcm_util.convert_codempid_to_temp(codempid||to_char(dtework, 'yyyymmdd')) key_check
      from tattence
     where codcomp  like p_codcomp || '%'
       and codcalen = p_codcalen
       --> Somchai || SEA-HR2201 || 23022023
  ...
  begin
    ...
    if v_codshift is not null then
      ...
      begin
        select codshift
          ...
          from tshifcom
         where codshift = v_codshift
           and codcompy = hcm_util.get_codcompy(v_codcomp);
      exception when no_data_found then
    ...
    if param_msg_error is null then
      ...
      for r1 in c_old_tattence loop
        json_dtestrtw.put(r1.key_check, to_char(r1.dtestrtw,'dd/mm/yyyy'));
        json_dteendw.put(r1.key_check, to_char(r1.dteendw,'dd/mm/yyyy'));
      ...
      for r1 in c_old_tattence loop
        begin
          select codcomp,typwork,codshift,dtestrtw,dteendw, hcm_util.convert_codempid_to_temp(codempid||to_char(dtework, 'yyyymmdd'))
        ...
        tattence_dtestrtw_o := to_date(hcm_util.get_string_t(json_dtestrtw, v_tmp_check),'dd/mm/yyyy');
        tattence_dteendw_o  := to_date(hcm_util.get_string_t(json_dteendw, v_tmp_check),'dd/mm/yyyy');
      ...
      for i in 0..param_json2.get_size-1 loop
        ...
        if v_codshift is not null or v_codshift <> '' then
          update tattence
             set codshift = v_codshift,
                 dtestrtw = nvl(v_dtestrtw, dtestrtw),
                 dteendw  = case when nvl(v_timstrtw,timstrtw) < nvl(v_timendw,dteendw) then
                              dtework
                            else
                              dtework + 1
                            end,
        ...
        if v_codshift is not null then
          ...
          begin
            select codshift
              ...
              from tshifcom
             where codshift = v_codshift
               and codcompy = hcm_util.get_codcompy(p_codcomp);
          exception when no_data_found then
            param_msg_error := get_error_msg_php('AL0061',global_v_lang,'tshifcom');
            json_str_output := get_response_message(null,param_msg_error,global_v_lang);
            return;
