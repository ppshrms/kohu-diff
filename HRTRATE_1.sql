--------------------------------------------------------
--  DDL for Package Body HRTRATE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRTRATE" is
-- last update: 25/05/2018 12:23
  procedure initial_value(json_str in clob) is
	...
	procedure getDestroy (json_str_input in clob, json_str_output out clob) is
		...
		obj_data		json_object_t;
		v_rcnt			number;
        p_codcompy      ttralert.codcompy%type;
		p_mailalno		ttralert.mailalno%type;
		p_dteeffec		ttralert.dteeffec%type;
	...
	begin
		...
		for i in 0..arr_destroy.get_size-1 loop
			...
			begin
				delete from ttralert where codcompy = p_codcompy and mailalno = p_mailalno and dteeffec = p_dteeffec;
				delete from ttrparam where codcompy = p_codcompy and mailalno = p_mailalno and dteeffec = p_dteeffec;
				delete from ttrasign where codcompy = p_codcompy and mailalno = p_mailalno and dteeffec = p_dteeffec;
				delete from tmailrtr where codcompy = p_codcompy and mailalno = p_mailalno and dteeffec = p_dteeffec;
			end;
		end loop;
  ...
  function get_seqno (v_codcompy varchar2, v_mailalno varchar2, v_dteeffec date) return number is
	...
	procedure saveData (json_str_input in clob, json_str_output out clob) as obj_row json_object_t;
        ...
        p_codsend               ttralert.codsend%type;
        p_code                  ttralert.syncond%type;
        p_statement             ttralert.statement%type;
		v_rcnt			        number := 0;
		...
		p_ffield		        ttrparam.ffield%type;
        p_descript              ttrparam.descript%type;
		p_codtable		        ttrparam.codtable%type;
		p_fparam		        ttrparam.fparam%type;
		p_flgdesc	            ttrparam.flgdesc%type;
        p_flgdesc_b             boolean;
		p_numseq		        ttrparam.numseq%type;
		v_numseq		        number := 0;
		p_flgDelete		        boolean;

        p_pfield                tmailrtr.pfield%type;
        p_pdesct                tmailrtr.pdesct%type;
        v_flgappr               ttrasign.flgappr%type;
        v_codcompap             ttrasign.codcompap%type;
        v_codposap              ttrasign.codposap%type;
        v_codempap              ttrasign.codempap%type;
        v_message               ttrasign.message%type;
...
--            param_msg_error := get_error_msg_php('HR3007',global_v_lang,get_label_name('HRTRATE2',global_v_lang,'360'));
            param_msg_error := get_error_msg_php('HR3007',global_v_lang);
            json_str_output := get_response_message(null,param_msg_error,global_v_lang);
            return;
        end if;
            ...
            if p_flgDelete then
                ...
                begin
                    ...
                    if sql%rowcount = 0 then
                        ...
                        insert into ttrparam (codcompy, mailalno,dteeffec,numseq,fparam,ffield,descript,
        ...
        begin
			INSERT INTO ttralert (codcompy,mailalno,dteeffec,typemail,flgeffec,subject, message,syncond,codsend,qtydayb, qtydayr,statement,codcreate,coduser)
					...
					begin
						...
						update ttrasign
						 ...
						 where codcompy = p_codcompy
                           ...
                           and seqno = v_seqno;
						if sql%rowcount = 0 then
							insert into ttrasign (codcompy, mailalno, dteeffec, seqno,flgappr,codcompap,codposap,codempap,message,coduser,codcreate)
        ...
        if v_count_assign = 0 then
            param_msg_error := get_error_msg_php('HR2045',global_v_lang,get_label_name('HRTRATE2',global_v_lang,'360'));
            json_str_output := get_response_message(null,param_msg_error,global_v_lang);
            rollback;
            return;
        end if;
				...
				if p_flgDelete then
					...
					begin
						...
						if sql%rowcount = 0 then
                            ...
                            insert into tmailrtr (codcompy, mailalno, dteeffec, pfield,
        ...
        if v_count_report = 0 then
            param_msg_error := get_error_msg_php('HR2045',global_v_lang,get_label_name('HRTRATE2',global_v_lang,'370'));
            json_str_output := get_response_message(null,param_msg_error,global_v_lang);
            rollback;
            return;
        end if;
	...
	procedure genDetail (json_str_output out clob) is
        ...
        v_row			number := 0;
        v_desc			varchar2(200 char);
        v_message_display ttralert.message%type;
        v_dteeffec      date;
	...
	procedure post_delete_report_format (json_str_input in clob, json_str_output out clob) as
        ...
        p_mailalno          tmailrtr.mailalno%type;
        p_flgDelete         boolean;
        p_flgdesc           tmailrtr.flgdesc%type;
        p_numseq            tmailrtr.numseq%type;
        p_pfield            tmailrtr.pfield%type;
        p_pdesct            tmailrtr.pdesct%type;
	begin
		initial_value(json_str_input);
	...
	begin
		...
		if v_number_mail > 0 then
			case p_typemail
				when '1' then alertmsg_tr.gen_typemail1(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
				when '2' then alertmsg_tr.gen_typemail2(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
                when '3' then alertmsg_tr.gen_typemail3(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
                when '4' then alertmsg_tr.gen_typemail4(p_codcompy,'N',p_mailalno,p_dteeffec,'N');
			end case;
