--------------------------------------------------------
--  DDL for Package Body HRPY35B
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRPY35B" as
...
--    end if;
--
--    v_filename := download.chk_file(get_tsetup_value('PATHWORK')||'\temp\'||lower(:global.v_form||'_'||:global.v_coduser)||'.log'
--                                    ,lower(:global.v_form||'_'||:global.v_coduser),1);
--    std_deltemp.upd_ttempfile(lower(:global.v_form||'_'||:global.v_coduser)||'.log','A');	--'A' = Insert , update ,'D'  = delete
--
--    out_file := Text_IO.Fopen(v_filename, 'w');
    ...
    for r_tgltrans in c_tgltrans loop
      ...
      --Text_IO.Put_line(out_file,data_file);
...
--         and dteyrepay = p_dteyrepay
--				 and dtemthpay = p_dtemthpay
--				 and stddec(amtsoca,codempid,v_chken) > 0
...
--redmine #1934
  ...
  begin
    ...
    --    obj_data.put('lastperiod', nvl(v_numperiod,0)||'/'||nvl(v_dtemthpay,0)||'/'||nvl(v_dteyrepay,0));
