--------------------------------------------------------
--  DDL for Package Body HRPY41B
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRPY41B" as
  ...
  begin
    ...
    p_flglast           := hcm_util.get_string_t(json_obj,'p_flglast');
 ...
 cursor c_tdtepay_next is
  ...
  begin
    ...
    if p_codempid is not null then
      ...
      begin
        select '1'
          into v_chk
          from ttaxcur
         where dteyrepay||lpad(dtemthpay,2,'0')||numperiod >
               p_dteyrepay||lpad(p_dtemthpay,2,'0')||p_numperiod
           and codcomp      like p_codcomp||'%'
           and typpayroll   = p_typpayroll
           and rownum       = 1;
        param_msg_error := get_error_msg_php('PY0075', global_v_lang);
        return;
    ...
    if v_currperd <> v_lastperd then
        ...
        if v_currperd <> v_nextperd then
            param_msg_error :=  get_error_msg_php('HR7518', global_v_lang)  ;--44B not check
            return;
        end if;
  end if;

	-- Find Control Data from PM Module about local currency, qty.hour
	/*begin -- v11 ไม่มี tcontpm แล้ว
    select codcurr
    ...
    select codpaypy1,codpaypy2,codpaypy3,
		...
		into ptcontrpy_codpaypy1,ptcontrpy_codpaypy2,ptcontrpy_codpaypy3,
				 ...
				 ptcontrpy_codpaypy8,ptcontrpy_codtax,ptcontpm_codcurr
		from tcontrpy --<< hrpy15e
		where codcompy = b_var_codcompy
		  and dteeffec = (select max(dteeffec)
											from tcontrpy
											where	codcompy = b_var_codcompy
...
procedure get_process (json_str_input in clob, json_str_output out clob) is
  ...
  procedure  insert_data_parallel (p_codapp  in varchar2,
	...
	v_numproc  number := 0; -- user22 : 10/12/2021 : ST11 || v_numproc  number;
	v_rec      number ;
...
begin
      ...
      delete ttemfilt where codapp like 'HRPY41%' and coduser = p_coduser  ; commit;
      commit ;
      begin
  ...
  procedure process_data(json_str_output out clob) is
    ...
    p_codapp        varchar2(100 char) := 'HRPY41B';
  ...
  begin
    ...
    std_tax.process_ ('HRPY41B',global_v_coduser,
    ...
    if o_numerr > 0 then
        for i in c_error loop
            ...
            obj_data2.put('image', get_emp_img(v_empid));
            ...
            hcm_batchtask.insert_batch_detail(
              ...
              p_codalw   => global_v_batch_codalw,
              p_dtestrt  => global_v_batch_dtestrt,
              p_item01  => get_emp_img(v_empid),
              p_item02  => v_empid,
              p_item03  => get_temploy_name(v_empid, global_v_lang),
              p_item04  => i.item03
            );
        end loop;
    end if;
   ...
   if nvl(o_numrec,0) > 0 or nvl(o_numerr,0) > 0 then
    ...
    if not v_exist then
        ...
        param_msg_error := get_error_msg_php('HR2715', global_v_lang);
        update tdtepay set  flgcal = 'Y', ----not update 44b
...
*/
  ...
  procedure get_last_date_process(json_str_input in clob, json_str_output out clob) is
    obj_data        json_object_t;
    v_response      varchar2(4000 char);
    ...
    if p_codempid is not null then
      begin
        select codcomp,typpayroll
          into p_codcomp,b_var_typpayroll
          from temploy1
         where codempid = p_codempid;
  ...
  procedure gen_latest_tax_cal(json_str_output out clob) is
    obj_data        json_object_t;
    v_response      varchar2(4000 char);
  ...
  procedure get_latest_tax_cal(json_str_input in clob,json_str_output out clob) is
    begin
        initial_value(json_str_input);
        if param_msg_error is null then
            gen_latest_tax_cal(json_str_output);
        else
            json_str_output := get_response_message(null,param_msg_error,global_v_lang);
        end if;
    exception when others then
