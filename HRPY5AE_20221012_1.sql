--------------------------------------------------------
--  DDL for Package Body HRPY5AE_20221012
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRPY5AE_20221012" as
-----
...
msg_err('save_data  t_tloanslf.codremark = '||t_tloanslf.codremark);
--<<user20 Date : 12/10/2022 #8418
...
msg_err('generate_column  v_head(7) = '||v_head(7));
msg_err('generate_column  v_column(7) = '||v_column(7));

    --
    param_import  := hcm_util.get_json_t(json_object_t(json_str_input),'params_json');
    obj_row       := json_object_t();
    for i in 0..(param_import.get_size - 1) loop
msg_err('generate_column  i = '||i);
      obj_data      := json_object_t();
      obj_data.put('coderror','200');
...
msg_err('generate_column  k = '||k);
msg_err('generate_column  v_text(k) = '||v_text(k));
msg_err('generate_column  v_column(k) = '||v_column(k));
