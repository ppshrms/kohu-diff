--------------------------------------------------------
--  DDL for Package Body M_HRMS6LU
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "M_HRMS6LU" is
/* Cust-Modify: KOHU-HR2301 */
-- last update: 19/10/2023 17:04
  ...
  begin
    ...
    -- surachai mo
    p_codcompbg         := hcm_util.get_string_t(json_obj, 'p_codcompbg');
    p_pctbguse          := hcm_util.get_string_t(json_obj, 'p_pctbguse');
  ...
  procedure hrms6lu (json_str_input in clob, json_str_output out clob) is
    ...
    -- mo surachai | 23/08/2023
    v_desPctbguse    varchar2(4000 char);
    v_qtyapp        number;
    ...
    CURSOR c_hrms6lu_c1 IS
       select  codempid,dtereq,numseq,dtestrt,dteend,
               ...
               0 qtyapp, --KOHU-HR2301 19/10/2023 ||b.approvno qtyapp,
               remark,a.qtyminb,a.qtymind,a.qtymina, a.staovrot,a.pctbguse
               --KOHU-HR2301 19/10/2023 cancel ||,a.routeno
         FROM  ttotreq a
               --KOHU-HR2301 19/10/2023 cancel ||,twkflowh b
         where /*KOHU-HR2301 19/10/2023 cancel
...
--         and
         ...
         and   ('Y' = chk_workflow.check_privilege('HRES6KE',codempid,dtereq,numseq,(nvl(a.approvno,0) + 1),v_codappr)
          ...
          --KOHU-HR2301 19/10/2023 cancel ||and a.routeno = b.routeno(+) --KOHU-HR2301 16/10/2023 add (+) ||and a.routeno = b.routeno
  ...
  begin
    ...
    if p_staappr = 'P' then
      ...
      /*--<<KOHU-HR2301 19/10/2023 cancel
        --<<KOHU-HR2301 16/10/2023
        if r1.routeno is null then
          begin
            select max(approvno) into v_qtyapp
            from   tempaprq
          ...
          if nvl(r1.appno,0)+1 = v_qtyapp then
             v_chk := 'E' ;
          else
             v_chk := v_appno;
          end if;
      ...
      if nvl(r1.appno,0)+1 = r1.qtyapp then
           ...
           v_chk := v_appno;
      --KOHU-HR2301 19/10/2023 cancel  ||end if;
        ...
        -- mo surachai | 23/08/2023
        if r1.pctbguse <= 100 then
            v_desPctbguse := get_label_name('HRMS6LU1',global_v_lang,'260');
        elsif r1.pctbguse > 100 then
            v_desPctbguse := get_label_name('HRMS6LU1',global_v_lang,'250');
        else
            v_desPctbguse := get_label_name('HRMS6LU1',global_v_lang,'9999');
        end if;
...
-- >> KOHU-HR2301 | 000504-Tae-Surachai-Dev | 18/04/2024 | 4449#1907 update ตาม std (ทำให้ระบบช้า)
      ...
      for r1 in c_hrms6lu_c2 loop
        ...
        -- mo surachai | 23/08/2023
--        if r1.pctbguse <= 100 then
--            v_desPctbguse := get_label_name('HRPY5JXC4',global_v_lang,'10');
--        elsif r1.pctbguse > 100 then
--            v_desPctbguse := get_label_name('HRPY5JXC4',global_v_lang,'20');
--        else
--            v_desPctbguse := get_label_name('HRPY5JXC4',global_v_lang,'30');
--        end if;
        --
        if r1.pctbguse <= 100 then
            v_desPctbguse := get_label_name('HRMS6LU1',global_v_lang,'260');
...
--
  ...
  procedure hrms6lu_detail_tab1 (json_str_input in clob, json_str_output out clob) is
    ...
    --
    v_codcompbg   ttotreq.codcompbg%type;
    v_qtybudget   ttotreq.qtybudget%type;
    v_qtyothot    ttotreq.qtyothot%type;
    v_pctbguse    ttotreq.pctbguse%type;
    -- mo surachai | 23/08/2023
    v_desPctbguse    varchar2(4000 char);
    v_dtestrt       date;
    v_dteend        date;
...
--<< user22 : 14/03/2024 : KOH-HR2301 #1767 ||
        v_dtestrtwk      := std_ot.get_dtestrt_period (r_ttotreq.codempid ,r_ttotreq.dtestrt);
        --v_dtestrtwk     := std_ot.get_dtestrt_prdcomp(r_ttotreq.codcompw, r_ttotreq.dtestrt);
-->> user22 : 14/03/2024 : KOH-HR2301 #1767 ||

        v_dteendwk      := v_dtestrtwk + 6;

        v_dtestrt       := least(v_dtestrtwk, r_ttotreq.dtestrt);
        IF TO_CHAR(r_ttotreq.dtestrt, 'MM') != TO_CHAR(v_dtestrt, 'MM') THEN
            v_dtestrt := TRUNC(r_ttotreq.dtestrt, 'MM');
        END IF;
        v_dteend        := greatest(v_dteendwk, trunc(r_ttotreq.dtestrt,'mm'));
        IF TO_CHAR(r_ttotreq.dtestrt, 'MM') != TO_CHAR(v_dteend, 'MM') THEN
            v_dteend := LAST_DAY(r_ttotreq.dtestrt);
        END IF;
    ...
    -- << surachai mo | 23/08/2023
    obj_data.put('des_codcompbg', get_tcenter_name(r_ttotreq.codcompbg,global_v_lang)); -- งบประมาณของหน่วย
    obj_data.put('wkbudgetdate',to_char(v_dtestrt,'dd/mm/yyyy')||' - '||to_char(v_dteend,'dd/mm/yyyy'));
    obj_data.put('qtybudgetw', hcm_util.convert_minute_to_hour(nvl(r_ttotreq.qtybudget,0))); -- งบประมาณประจำสัปดาห์ วันที่
    obj_data.put('qtyotreq', hcm_util.convert_minute_to_hour(nvl(r_ttotreq.qtyotreq,0))); -- จำนวนชั่วโมงล่วงเวลาในใบคำขอนี้
    obj_data.put('qtyothot', hcm_util.convert_minute_to_hour(nvl(r_ttotreq.qtyothot,0))); -- จำนวนชั่วโมงล่วงเวลาในใบคำขออื่น
    obj_data.put('qtyotall', hcm_util.convert_minute_to_hour(nvl(r_ttotreq.qtyotreq,0) + nvl(r_ttotreq.qtyothot,0))); -- รวมจำนวนชั่วโมงล่วงเวลา
    v_qtybudget := nvl(r_ttotreq.qtybudget,0) -(nvl(r_ttotreq.qtyotreq,0) + nvl(r_ttotreq.qtyothot,0));
    if v_qtybudget < 0 then
        v_qtybudget := 0;
    end if;
    obj_data.put('qtybudget', hcm_util.convert_minute_to_hour(nvl(v_qtybudget,0))); -- จำนวนชั่วโมงของงบประมาณคงเหลือ
    obj_data.put('pctbguse', r_ttotreq.pctbguse || '%'); -- % ใช้ไป
    if r_ttotreq.pctbguse <= 100 then
        v_desPctbguse := get_label_name('HRMS6LU1',global_v_lang,'240');
    elsif r_ttotreq.pctbguse > 100 then
        v_desPctbguse := get_label_name('HRMS6LU1',global_v_lang,'230');
    else
        v_desPctbguse := get_label_name('HRMS6LU1',global_v_lang,'999');
    end if;
    obj_data.put('des_pctbguse', v_desPctbguse); -- % ใช้ไป
    -- >>
       begin
...
--            if p_ttotreq.timastr is not null or p_ttotreq.timaend is not null or p_ttotreq.qtymina is not null then
  ...
  begin
    ...
    -- << surachai mo | 23/08/2023
     begin
          select codcompbg, pctbguse,dtestrt
          --,qtystepappr
          into p_codcompbg,p_pctbguse,p_dtestrt
          -- ,p_qtystepappr
          from  ttotreq
          where codempid =  rq_codempid
          and   dtereq   =  rq_dtereq
    ...
    --->>> weerayut 31/01/2018
    --<<KOHU-HR2301 16/10/2023
    --KOHU-HR2301 19/10/2023 cancel ||if v_ttotreq.routeno is null then
      begin
        select max(approvno) into v_max_approv
        from   tempaprq
        where  codapp    = 'HRES6KE'
        and    codempid  = v_ttotreq.codempid
        and    dtereq    = v_ttotreq.dtereq
        and    numseq    = v_ttotreq.numseq;
      end;
    /*KOHU-HR2301 19/10/2023 cancel
    else
    -->>KOHU-HR2301 16/10/2023
    ...
    if v_ttotreq.staappr <> 'Y' then
      ...
      /*KOHU-HR2301 19/10/2023 cancel
      if v_ttotreq.routeno is not null then --KOHU-HR2301 16/10/2023

      chk_workflow.find_next_approve('HRES6KE',v_routeno,rq_codempid,to_char(rq_dtereq,'dd/mm/yyyy'),rq_seqno,v_approvno,p_codappr);
--      chk_workflow.find_next_approve('HRES6KE',v_routeno,rq_codempid,to_char(rq_dtereq,'dd/mm/yyyy'),rq_seqno,v_approvno,p_codappr,p_qtystepappr); -- surachai mo | 28/08/2023 (ไม่ใช้งานแล้วเนื่องจากไม่มีการ mo ของ approve)
