--------------------------------------------------------
--  DDL for Package Body HRAL85B_BATCH_OFF
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRAL85B_BATCH_OFF" is
...
procedure start_process is
  ...
  procedure cal_process(p_codempid    in  varchar2,
    ...
    a_qtyminot    HRAL85B_BATCH_OFF.a_qtyminot;
    a_rteotpay    HRAL85B_BATCH_OFF.a_rteotpay;
      ...
      select a.numotreq,a.codrem,a.codappr,a.dteappr,
       ...
       where a.numotreq   = b.numotreq
         ...
         and((p_coduser   = 'AUTOBATCH' and b.dayeupd   is null
                                        and b.dtecreate between (v_sysdate - 1) and v_sysdate)--user22 : ST11 : 02/11/2021 || and b.dtewkreq  <= v_sysdate - 1)
...
-->> user22 : ST11 : 16/08/2021 || delete tovrtime, totpaydt
  ...
  t_datein1     date; -- user22 : 14/02/2020 : OFFICE
  t_datein2     date; -- user22 : 14/02/2020 : OFFICE
...
-->> user22 : 29/09/2021 :
      ...
      -- Check Time Start After
      if p_typot = 'A' and p_chkwkfull = 'Y' and rt_tshiftcd.qtywkfull > 0 then
      ...
      end if;-- p_typot = 'A' and p_chkwkfull = 'Y' and rt_tshiftcd.qtywkfull > 0
--<< user22 : 14/02/2020 : OFFICE
      if p_typot = 'A' then
        t_datein1 := to_date(to_char(v_dtein,'dd/mm/yyyy')||v_timin,'dd/mm/yyyyhh24mi');
        t_datein1 := greatest(t_datein1,v_strtw);
        t_datein2 := to_date(to_char(v_dtein,'dd/mm/yyyy')||'0915','dd/mm/yyyyhh24mi');
        if t_datein1 > t_datein2 then
          v_strtot := t_datein1 + .375 + (15 / 1440);
        end if;
      end if;
-->> user22 : 14/02/2020 : OFFICE 
/*
      -- Check Time Start After
      if p_typot = 'A' and rt_tshiftcd.qtywkfull > 0 then--boy
        v_datein := to_date(to_char(v_dtein,'dd/mm/yyyy')||v_timin,'dd/mm/yyyyhh24mi');
        v_datein := greatest(v_datein,v_strtw);
        if v_datein between v_strtb and v_endb then
          v_datein  := v_endb;
          v_dateout := v_datein + (rt_tshiftcd.qtywkfull / 1440);
        else
          v_dateout := v_datein + (rt_tshiftcd.qtywkfull / 1440);
          v_dedbrk  := std_al.Cal_Min_Dup(v_strtb,v_endb,v_datein,v_dateout);
          v_dateout := v_dateout + (v_dedbrk / 1440);
        end if;
        v_strtot := greatest(v_strtot,v_dateout);
      end if;
*/
      --
...
--insert into  a (a) values  ( ' r_totratep2 '|| p_rteotpay(i) ||' - '||p_qtyminot(i)) ; commit;
        ...
        if nvl(v_qtyminall,0) > 0 then
          ...
          --if not(v_typotreq = '2' and nvl(v_qtymincal,0) > 0 and nvl(v_qtymincal,0) > v_qtyminall) then
          if not(nvl(v_qtymincal,0) > 0 and nvl(v_qtymincal,0) > v_qtyminall) then
