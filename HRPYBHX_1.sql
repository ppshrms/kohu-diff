--------------------------------------------------------
--  DDL for Package Body HRPYBHX
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRPYBHX" as
  ...
  procedure gen_index(json_str_output out clob) as
    ...
    cursor c1 is
      select dteedit    ,numpage    ,fldedit ,
             ...
             a.codempid ,a.rowid     rowids, b.codpfinf ,b.codcomp
        from tpfmlog a,tpfmemb b
--        ,temploy1 c
...
--         and a.fldedit not in ('CODEMPID','DTEEFFEC')
  ...
  begin
    ...
    for r1 in c1 loop
      ...
      if v_flg_secure then
        ...
        obj_data.put('olddata'       ,get_description(r1.codempid, r1.codtable,r1.fldedit,r1.desold));
        obj_data.put('newdata'       ,get_description(r1.codempid, r1.codtable,r1.fldedit,r1.desnew));
  ...
  function get_description(p_codempid in varchar2, p_table in varchar2,p_field in varchar2,p_code in varchar2) return varchar2 as
  ...
  begin
    ...
    if v_funcdesc is not null then
      ...
      if v_data_type = 'DATE' then
       ...
       -- return  v_desc;
            if p_field like 'AMT%' then 
                begin
                     v_chk_data := to_number(p_code);
                     v_chk_num := 'Y';
                     exception when others  then 
                      v_chk_num := 'N';
                end;
                ...
                if v_chk_num = 'Y' then 
                    ...
                    v_desc := to_char((stddec(p_code,p_codempid,global_v_chken)),'fm9,999,999,999,990.00');
                end if;
            else
               v_desc := p_code;
            end if;            
