--------------------------------------------------------
--  DDL for Package Body HRAL1NX
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HRAL1NX" as
...
*/
  ...
  procedure get_groupplan(json_str_input in clob, json_str_output out clob) is
    ...
    v_codcomp_print   temploy1.codcomp%type;
    v_codcalen_print  temploy1.codcalen%type;
    cursor c_tgrpplan is
		...
		--<< STA4620390 02/11/2019
		and		exists(	select  a.codempid
                          from  temploy1 a,tattence b
                         where  a.codcomp 	like p_codcomp||'%'
                --           and  p.codcomp 	= get_tgrpwork_codcomp(get_tattence_codcomp(a.codempid,v_stdate,v_endate),p.codcalen)
                          and  p.codcomp = (select  max(e.codcomp)
                                             from   tgrpwork e
                                             where  b.codcomp like e.codcomp||'%'
                                             and    e.codcalen = b.codcalen
                                             and    e.dteeffec = ( select max(dteeffec)
                                                                   from   tgrpwork d
                                                                   where  e.codcomp   = d.codcomp
                                                                   and    e.codcalen  = d.codcalen
                                                                   and    d.dteeffec <= trunc(sysdate)
                                                                ))

                           and  a.codempid = b.codempid
                           and  b.dtework  between v_stdate and v_endate
                           and  b.codcalen = p.codcalen
                           and 	rownum = 1 --**
								)
    ...
    if param_msg_error is null then
      ...
      if isInsertReport then
        ...
        for i in 0..(param_json.get_size - 1) loop
          ...
          v_codcomp_print    := hcm_util.get_string_t(param_json_row,'codcomp');
          v_codcalen_print   := hcm_util.get_string_t(param_json_row,'codcalen');
          v_total := 1;
          param_msg_error := hcm_secur.secur_codcomp(global_v_coduser, global_v_lang, v_codcomp_print);
          if param_msg_error is not null then
            exit;
          end if;
          v_codcomp  := v_codcomp_print;
          v_codcalen := v_codcalen_print;
          if v_codcomp_print <> o_codcomp then
            o_codcalen := '!@#$';
            v_day := 0;
            v_first := true;
          end if;
          for r2 in c_tgrpplan2 loop
            ...
            if r2.codcalen <> o_codcalen then
              ...
              obj_data.put('dtework', nvl(to_char(r2.dtework,'dd/mm/yyyy'), ''));
              begin
            ...
            obj_data.put('typwork'||lpad(to_char(r2.dtework,'dd'),2,'0'), nvl(r2.typwork, ''));
            obj_data.put('codshift'||lpad(to_char(r2.dtework,'dd'),2,'0'), nvl(r2.codshift, ''));
            o_codcalen := r2.codcalen;
            o_codcomp  := r2.codcomp;
          end loop;
          -- codcomp,codcalen
          select count(distinct b.codempid)  into v_emp_count
          from tattence b ,temploy1 a
          where b.dtework between v_stdate and v_endate
          and  b.codempid = a.codempid
          and  b.codcalen = v_codcalen_print
          and  v_codcomp_print = (select  max(e.codcomp)
                                   from   tgrpwork e
                                   where  b.codcomp like e.codcomp||'%'
                                   and    e.codcalen = b.codcalen
                                   and    e.dteeffec = ( select max(dteeffec)
          ...
          begin
            select nvl(count(codempid), 0)
              into v_emp_count
              from temploy1 a
             where v_codcomp_print = get_tgrpwork_codcomp(get_tattence_codcomp(codempid, v_stdate, v_endate), null)
               and exists (select b.codcalen
                             from tattence b
                            where a.codempid = b.codempid
                              and b.dtework between v_stdate and v_endate
                              and codcalen = v_codcalen_print)
               and hcm_secur.secur2_cursor(global_v_coduser, global_v_zminlvl, global_v_zwrklvl, codempid) = 'Y';
          end;
          */

          obj_data.put('emp_count', to_char(v_emp_count,'fm999,999,999'));
          obj_row.put(to_char(v_row-1),obj_data);
        end loop;
        commit;
      else
        for r1 in c_tgrpplan loop
          ...
          begin
            select count(distinct b.codempid)  into v_emp_count
            from   tattence b ,temploy1 c
            where  b.dtework between v_stdate and v_endate
            and    b.codcalen = r1.codcalen
            and    b.codempid  = c.codempid
            and    r1.codcomp = (select  max(e.codcomp)
                                   from   tgrpwork e
                                   where  b.codcomp like e.codcomp||'%'
                                   and    e.codcalen = b.codcalen
                                   and    e.dteeffec = ( select max(dteeffec)
  ...
  procedure gen_groupemp(json_str_output out clob, p_cnt_emp number default 0) is
    obj_row       json_object_t;
    obj_data      json_object_t;
    v_stdate		  date;
    v_endate		  date;
    ...
    cursor c_temploy1 is
      ...
      where  --get_tgrpwork_codcomp(get_tattence_codcomp(codempid,v_stdate,v_endate),null) = p_codcomp
           p_codcomp = (select  max(e.codcomp)
                                   from   tgrpwork e
                                   where  a.codcomp like e.codcomp||'%'
                                   --and    e.codcalen = b.codcalen
                                   and    e.dteeffec = ( select max(dteeffec)
                                                         from   tgrpwork d
                                                         where  e.codcomp   = d.codcomp
  ...
  begin
    if param_msg_error is null then
      ...
      for r1 in c_temploy1 loop
        ...
        if v_flgsecu then
          v_secure := v_secure + 1; --10/03/2021
...
--        obj_data := json_object_t();
--        obj_data.put('codempid', get_label_name('HRAL1NX', global_v_lang, '490'));
...
--        v_numseq := v_numseq + v_emp_count;
        if param_msg_error is not null then
          ...
          check_index;
          if param_msg_error is null then
            gen_groupemp(json_str_output, v_cnt_emp);
          else
            return;
          end if;
        end if;
  ...
  procedure insert_ttemprpt_comp(v_numseq in number, obj_data in json_object_t) is
    ...
    v_item74            ttemprpt.item74%type;
    v_item75            ttemprpt.item75%type;
    v_item76            ttemprpt.item76%type;
    v_item77            ttemprpt.item77%type;
    v_item78            ttemprpt.item78%type;
  ...
  begin
    ...
    v_item77 := get_tcenter_name(p_codcomp, global_v_lang);
    v_item78 := get_tcodec_name('TCODWORK', p_codcalen, global_v_lang);
    v_emp_count := to_number(hcm_util.get_string_t(obj_data, 'cntemp'));
    ...
    begin
      ...
      values
          (
            ...
            v_item71, p_year, p_month_insert, p_codcomp, p_codcalen, v_emp_count,
            v_item77, v_item78
          );
  ...
  begin
    ...
    --day name--
    v_item38  := get_label_name('HRAL1NXC1', global_v_lang, '10');
    v_item39  := get_label_name('HRAL1NXC1', global_v_lang, '20');
    v_item40  := get_label_name('HRAL1NXC1', global_v_lang, '30');
    v_item41  := get_label_name('HRAL1NXC1', global_v_lang, '40');
    v_item42  := get_label_name('HRAL1NXC1', global_v_lang, '50');
    v_item43  := get_label_name('HRAL1NXC1', global_v_lang, '60');
    v_item44  := get_label_name('HRAL1NXC1', global_v_lang, '70');
